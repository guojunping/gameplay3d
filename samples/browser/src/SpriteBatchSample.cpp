...
void SpriteBatchSample::render(float elapsedTime)
{
    ...
    for (unsigned int i = 0; i < pointCount; i++)
    {
        ...
        {
            _spriteBatch->draw(Rectangle(x, y, 32, 32), src, Vector4::fromColor(0x0073BCFF));
            x += 32;    
        }
        x = 0;
        y += 32;
	...
	_spriteBatch->draw(Rectangle(x + 512, y - 512, 512, 512), src, Vector4::fromColor(0x0073BCFF));
    _spriteBatch->finish();
