...
Font* Bundle::loadFont(const char* id)
{
	...
	unsigned int isSupportUnicode;
	if (_stream->read(&isSupportUnicode, 4, 1) != 1)
	{
		GP_ERROR("Failed to read support unicode for font '%s'.", id);
		return NULL;
	}
	...
	if (isSupportUnicode == 0) 
	{
		...
		for (unsigned int i = 0; i < fontSizeCount; ++i)
		{
			...
			Font* font = AFont::create(family.c_str(), Font::PLAIN, size, glyphs, glyphCount, texture, (Font::Format)format);
	...
	{
		unsigned int *fontSizeArray = new unsigned int[fontSizeCount];
		for (unsigned int i = 0; i < fontSizeCount; ++i)
		{
			if (_stream->read(&fontSizeArray[i], 4, 1) != 1)
			{
				GP_ERROR("Failed to read fontSize for font '%s'.", id);
				return NULL;
			}
		}

		int fontDataLen;
		if (_stream->read(&fontDataLen, 4, 1) != 1)
		{
			GP_ERROR("Failed to read length for font '%s'.", id);
			return NULL;
		}

		char* fontData = new char[fontDataLen+1];
		int readSize = 0;
		do {
			int fileDataSize = _stream->read(&fontData[readSize], 1, 4096);
			if (fileDataSize <= 0)
			{
				GP_ERROR("Failed to read data for font '%s'.", id);
				return NULL;
			}
			readSize += fileDataSize;
		} while (readSize < fontDataLen);
		
		fontData[fontDataLen] = 0x0;

		Font* masterFont = NULL;
		for (int i = 0; i < fontSizeCount; i++) {
			// Create the font for this size
			Font* font = UFont::create(fontData, fontDataLen, fontSizeArray[i], Font::BITMAP, masterFont);
			font->_path = _path;
			font->_id = id;
