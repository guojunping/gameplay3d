...
void SpriteBatch::setTexture(Texture* texture) {
	...
	Material* material = _batch->getMaterial();
	...
	SAFE_RELEASE(this->_sampler)

	this->_sampler = sampler;
	this->_textureWidthRatio = 1.0f / (float)texture->getWidth();
	this->_textureHeightRatio = 1.0f / (float)texture->getHeight();
