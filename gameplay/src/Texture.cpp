...
void Texture::setData(int offx, int offy, int w, int h, const unsigned char* data)
{
	// Don't work with any compressed or cached textures
	GP_ASSERT(data);
	GP_ASSERT((!_compressed));
	...
	if (_type == Texture::TEXTURE_2D)
	{
		GL_ASSERT(glTexSubImage2D(GL_TEXTURE_2D, 0, offx, offy, w, h, _internalFormat, _texelType, data));
	}
	else
	{
		// Get texture size
		...
		for (unsigned int i = 0; i < 6; i++)
		{
			GL_ASSERT(glTexSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, offx, offy, w, h, _internalFormat, _texelType, &data[i * textureSize]));
		}
	}
...
void Texture::copyTexture(Texture *srcTex, int offx, int offy, int x, int y, int w, int h) {
	GLuint fbo; 
	///
