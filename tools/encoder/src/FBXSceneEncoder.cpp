...
void FBXSceneEncoder::write(const string& filepath, const EncoderArguments& arguments)
{
	...
	_fileName = getFilenameNoExt(fileName);
...
void FBXSceneEncoder::loadScene(FbxScene* fbxScene)
{
    ...
    if (rootNode)
    {
        ...
        for (int i = 0; i < childCount; ++i)
        {
            ...
            if (node)
            {
				//LOG(1, "node id = %s ,type = %u \n", node->getId().c_str(),node->getTypeId());
                scene->add(node);
            }
        }
    }
...
void FBXSceneEncoder::loadAnimationLayer(Animation* animation , FbxAnimLayer* fbxAnimLayer, FbxNode* fbxNode, const EncoderArguments& arguments)
{
    ...
    {
        loadAnimationLayer(animation, fbxAnimLayer, fbxNode->GetChild(modelCount), arguments);
    }
}
...
void FBXSceneEncoder::loadAnimations(FbxScene* fbxScene, const EncoderArguments& arguments)
{
	...
	LOG(1,"number of animation stacks: %d\n", animStackCount);
    ...
    for (int i = 0; i < animStackCount; ++i)
    {
		...
		LOG(1,"animation stack %d\n", i);
		LOG(1,"	name: %s\n", takeinfo->mName.Buffer());
		LOG(1,"	description: %s\n", takeinfo->mDescription.Buffer());
		LOG(1,"	import name: %s\n", takeinfo->mImportName.Buffer());
		LOG(1,"	import state: %s\n\n", takeinfo->mSelect ? "true" : "false");

		FbxTime startTime = takeinfo->mLocalTimeSpan.GetStart();		//start time
		FbxTime stopTime = takeinfo->mLocalTimeSpan.GetStop();		//stop time
		fbxsdk::FbxString  &name = takeinfo->mName;

		Animation* animation = new Animation();
		if (animStackCount == 1) {
			if (arguments.getAnimationId().empty()) {
				animation->setId(name.Buffer());
			}
			else {
				animation->setId(arguments.getAnimationId());
			}
		}
		else {
			if (arguments.getAnimationId().empty()) {
				fbxsdk::FbxString  &name = takeinfo->mName;
				animation->setId(_fileName + "@" + name.Buffer());
			}
			else {
				animation->setId(arguments.getAnimationId() + "@" + name.Buffer());
			}
		}
		...
		for (int l = 0; l < nbAnimLayers; ++l)
		{
			...
			loadAnimationLayer(animation, animLayer, fbxScene->GetRootNode(), arguments);
		}
...
void FBXSceneEncoder::loadMaterial(FbxNode* fbxNode)
{
    ...
    for (int index = 0; index < materialCount; ++index)
    {
        ...
        else
        {
			...
			LOG(1, "nodeId = %s,material name = %s\n", node->getId().c_str(), materialName.c_str());
