...
static void writeString(FILE* fp, const char* str, unsigned int len)
{
	fwrite(&len, sizeof(unsigned int), 1, fp);
	if (len > 0)
	{
...
int writeUnicodeFont(const char* inFilePath, const char* outFilePath, std::vector<unsigned int>& fontSize, const char* id, Font::FontFormat fontFormat) {
	...
	FILE *fontFp = fopen(inFilePath, "rb");

	fseek(fontFp, 0, SEEK_END);
	int fontDataLen = ftell(fontFp);
	fseek(fontFp, 0, SEEK_SET);

	LOG(1, "write font file size %d. \n", fontDataLen);

	char* fontData = new char[fontDataLen];
	size_t fontFileSize = fread(fontData, 1, fontDataLen, fontFp);
	if (fontFileSize <= 0) {
		LOG(1, "read font file error: %d \n", fontFileSize);
		return -1;
	}
	...
	// Number of included font sizes (GPB version 1.3+)
	writeUint(gpbFp, (unsigned int)fontSize.size());

	for (int i = 0; i < fontSize.size(); i++) {
		writeUint(gpbFp, fontSize[i]);
	}

	writeString(gpbFp, fontData, fontFileSize);
