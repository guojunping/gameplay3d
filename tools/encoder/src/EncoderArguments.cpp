...
const std::string EncoderArguments::getAnimationId() const
{
    return _newAnimationId;
}
...
void EncoderArguments::printUsage() const
{
    ...
    "  -t\t\tWrite text/xml.\n" \
    "  -a <animation id>\n" \
        "\t\tSpecify a new ID for the animation. \n" \
...
bool EncoderArguments::fontSupportUnicode() const
{
	return _fontSupportUnicode;
}
...
void EncoderArguments::readOption(const std::vector<std::string>& options, size_t* index)
{
    ...
    case 'a':
        ...
        {
            LOG(1, "Error: -a requires 1 arguments.\n");
            _parseError = true;
            return;
        }
        (*index)++;
        _newAnimationId = options[*index];
...
void unittestsEncoderArguments()
{
    ...
    {
        const char* argv[] = {exe, "-a",  "movements", "C:\\Git\\gaming\\GamePlay\\gameplay-encoder\\res\\duck.fbx"};
