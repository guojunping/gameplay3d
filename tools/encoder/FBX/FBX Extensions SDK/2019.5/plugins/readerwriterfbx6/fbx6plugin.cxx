...
#include "fbxwriterfbx6.h"

#define PLUGIN_NAME			"CustomFBX6"
...
{
	FbxWriter* lWriter = FbxNew< FbxWriterFbx6 >(pManager, pExporter, pPluginID, FbxStatusGlobal::GetRef());
...
{
	FbxReader* lReader = FbxNew< FbxReaderFbx6 >(pManager, pImporter, pPluginID, FbxStatusGlobal::GetRef());
...
class Fbx6Plugin : public FbxPlugin
{
	FBXSDK_PLUGIN_DECLARE(Fbx6Plugin);

protected:
	explicit Fbx6Plugin(const FbxPluginDef& pDefinition, FbxModule pFbxModule) : FbxPlugin(pDefinition, pFbxModule)
	{
	}
...
FBXSDK_PLUGIN_IMPLEMENT(Fbx6Plugin);

// FBX Interface
extern "C"
{
    //The DLL is owner of the plug-in
    static Fbx6Plugin* sPlugin = NULL;
	...
	FBXSDK_DLLEXPORT void FBXPluginRegistration(FbxPluginContainer& pContainer, FbxModule pFbxModule)
    {
        if( sPlugin == NULL )
        {
            ...
            sPlugin = Fbx6Plugin::Create(sPluginDef, pFbxModule);
